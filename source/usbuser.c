...
{	
	...
	if(sHID_Data.wHID_SetReportFeature == mskSET_REPORTFEATURE_FLAG)	//** Check Set_report_feature == 1
	{
      ...
      if ((usbReceivelen >= 64) && (sUSB_EumeData.bUSB_wLength > 64)) {
			  sHID_Data.ReportFeature.wHID_Setreportfeature[16+i] = wUSBINT_ReadDataBuf;
      } else {
      ...
      (sUSB_EumeData.bUSB_wValueL == 0x00) && (sUSB_EumeData.bUSB_bmRequestType == 0x21) && (sUSB_EumeData.bUSB_bRequest == 0x09)) {
        usbReceivelen += 4;
//      } else {
//        sHID_Data.wHID_SetReportFeature = mskSET_REPROTFEATURE_ARRIVAL;	
     ...
     if ((usbReceiveFlag == 0) && (usbReceivelen>= PROTOCOL_DATA_SIZE)) {
      memcpy(protocolTransmit, (uint8_t *)&sHID_Data.ReportFeature.wHID_Setreportfeature[0], PROTOCOL_DATA_SIZE);
      usbReceiveFlag = 1;
      usbReceivelen = 0; 
      protocolTransmit[0] = 1;
    }   
